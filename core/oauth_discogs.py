...
def save_consumer_credentials(consumer_key: str, consumer_secret: str) -> None:
    """Persist bundled OAuth credentials for all users of this app build."""
    key = consumer_key.strip()
    secret = consumer_secret.strip()
    if not key or not secret:
        raise ValueError("Consumer key and secret are required.")

    secrets_path = project_root() / "core" / "discogs_oauth_secrets.py"
    secrets_path.write_text(
        (
            '"""\n'
            "Discogs OAuth consumer key and secret (local only — do not commit).\n"
            "Generated by setup. See OAUTH_SETUP.md.\n"
            '"""\n\n'
            "from __future__ import annotations\n\n"
            f'BUNDLED_DISCOGS_CONSUMER_KEY: str = "{key}"\n'
            f'BUNDLED_DISCOGS_CONSUMER_SECRET: str = "{secret}"\n'
        ),
        encoding="utf-8",
    )

    os.environ["DISCOGS_CONSUMER_KEY"] = key
    os.environ["DISCOGS_CONSUMER_SECRET"] = secret
    _ensure_dotenv_loaded()
